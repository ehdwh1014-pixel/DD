...
| Coolint P | NI **9477** `cDAQ2Mod3/port0/line4` | **DO 4** 24 V 2선 펌프 (SINK) |
...
주파수 지령만 쓰는 경우 FX(RUN)는 인버터 측에서 별도 처리합니다. **DO4는 Coolint P**용이며 피드백이 건드리지 않습니다.
...
### 5b) Coolint P DO4 → NI 9477

24 V 2선식 펌프. **Coolint P**는 메인 화면에서 ON/OFF합니다. 채널당 ≤1 A.
...
배선: **DI0 = H(황)**, **DI1 = L(청)**, **COM = 백**. HH/LL은 사용하지 않습니다.

4접점 리드스위치는 수위가 올라가면 아래 접점부터 순서대로 ON됩니다.  
따라서 **H ON이면 L도 항상 ON**이며, 이를 충돌로 보지 않습니다.

| 수위 | DI0 (H) | DI1 (L) | 급수 밸브 |
|------|---------|---------|-----------|
| 매우 낮음 | OFF | OFF | 열림 |
| L 높이 | OFF | ON | 열림 |
| H 높이 | ON | ON | **닫힘** |
...
- **Coolint P(DO4)**는 메인 화면 **통신 / 점화** 패널(또는 터치 **점화** 탭)에서 ON/OFF합니다.
...
- 팝업을 닫으면 AO2=0 V, DO0~2=OFF 후 레벨 자동제어로 복귀합니다. Coolint P(DO4) 상태는 유지됩니다.
...
- TC 읽기: **5 s**
- NI 장치 재검색: **5 s**
- 밸브 DO0~2: **상태가 바뀔 때만** DAQ 출력
- MP5Y: 최소 Modbus RTU (pyserial만 사용, pymodbus 없음)
...
- MP5Y가 끊겨도 NI-DAQ이 정상이면 레벨센서 자동 밸브 제어, 점화 DO, Coolint P(DO4), 메인 NG PUMP(AO1), I/O TEST의 AO2는 계속 사용할 수 있습니다.
...
2. 우측 상단 점화 / Coolint P — IGN·Coolint P 수동 버튼
3. 하단 레벨/밸브 — DAQ 연결 즉시 자동 운전, HIGH/LOW 입력과 밸브 명령
4. `I/O TEST`(AO2/DO0~2), `TC MON` — 별도 팝업
