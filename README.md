...
- **피드백 제어**: AI 전압을 유량 또는 압력으로 환산한 PV/SV 트렌드, AO 출력 트렌드, PI 제어 및 1차 LPF
...
- LPF 차단 주파수 (Hz, `0`이면 필터 미사용)

제어식은 `AO = clamp(P × (SV − PV) + I × ∫(SV − PV)dt, 0, 5 V)`입니다. PV는 AI 전압을 공정값으로 선형 환산한 뒤 LPF를 적용합니다. 출력 포화 시 적분값이 계속 쌓이지 않도록 anti-windup을 적용합니다.
